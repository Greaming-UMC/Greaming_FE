...
import { LoadingSpinner } from "../../../components/common/feedback/LoadingSpinner";
import { useNavigate, useSearchParams } from "react-router-dom";
import { setAccessToken } from "../../../libs/security/tokenStore";

const TRUE_SET = new Set(["true", "1", "yes", "y"]);
const FALSE_SET = new Set(["false", "0", "no", "n"]);

const pickToken = (sp: URLSearchParams) =>
  sp.get("accessToken") ??
  sp.get("access_token") ??
  sp.get("token") ??
  sp.get("jwt");
...
const AuthCallbackView = () => {
  ...
  useEffect(() => {
    const error = searchParams.get("error");
    const isNewUserRaw = searchParams.get("is_new_user") ?? searchParams.get("isNewUser");
    const normalized = (isNewUserRaw ?? "").toLowerCase();
